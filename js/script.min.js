"use strict";var tipCalculator={billValue:document.querySelector(".bill input"),tipSelection:document.querySelectorAll(".tipSelect input"),customTipValue:document.querySelector(".tipSelect input:nth-child(7)"),numberOfPeople:document.querySelector(".peopleNumber input"),spanInfo:document.querySelector(".peopleNumber p span"),tipAmountResult:document.querySelector(".tipAmount"),TotalAmountResult:document.querySelector(".total"),tipValue:0,resetButton:document.querySelector(".result button"),calculation:function(){this.tipAmountResult.innerHTML="$"+(parseInt(this.billValue.value)*parseInt(this.tipValue)*.01/parseInt(this.numberOfPeople.value)).toFixed(2),this.TotalAmountResult.innerHTML="$"+((parseInt(this.billValue.value)+parseInt(this.billValue.value)*parseInt(this.tipValue)*.01)/parseInt(this.numberOfPeople.value)).toFixed(2)},classClearing:function(){for(var t=0;t<tipCalculator.tipSelection.length;t++)tipCalculator.tipSelection[t].classList.remove("focused")},valueChange:function(){0!=tipCalculator.billValue.value&&0!=tipCalculator.tipValue&&0!=tipCalculator.numberOfPeople.value?this.calculation():(this.TotalAmountResult.innerHTML="$0",this.tipAmountResult.innerHTML="$0")},fieldsClearing:function(){this.TotalAmountResult.innerHTML="$0",this.tipAmountResult.innerHTML="$0",this.classClearing(),this.tipValue=0,this.billValue.value="0",this.numberOfPeople.value="0   ",this.customTipValue.value="",this.spanInfo.classList.remove("activeSpan"),this.numberOfPeople.classList.remove("activeInput")}};tipCalculator.tipSelection.forEach(function(t){t.addEventListener("click",function(t){tipCalculator.classClearing(),console.log(t.target),tipCalculator.tipValue=t.target.value,t.target.classList.add("focused"),tipCalculator.valueChange()})}),tipCalculator.customTipValue.addEventListener("input",function(t){tipCalculator.tipValue=t.target.value,tipCalculator.valueChange()}),tipCalculator.billValue.addEventListener("input",function(t){tipCalculator.valueChange()}),tipCalculator.numberOfPeople.addEventListener("input",function(t){tipCalculator.valueChange(),0==tipCalculator.numberOfPeople.value?(tipCalculator.spanInfo.classList.add("activeSpan"),tipCalculator.numberOfPeople.classList.add("activeInput")):(tipCalculator.spanInfo.classList.remove("activeSpan"),tipCalculator.numberOfPeople.classList.remove("activeInput"))}),tipCalculator.resetButton.addEventListener("click",function(){tipCalculator.fieldsClearing()});
